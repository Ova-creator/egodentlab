  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;line-height:1.45;color:#111">
    ...
    <tr><td>
      <table width="100%" cellpadding="8" cellspacing="0" style="border:1px solid #eee;border-radius:8px">
        <tr><td width="30%" style="background:#fafafa;border-bottom:1px solid #eee">Clinic</td><td style="border-bottom:1px solid #eee">${escape(clinic)}</td></tr>
        <tr><td style="background:#fafafa;border-bottom:1px solid #eee">Email</td><td style="border-bottom:1px solid #eee">${escape(email)}</td></tr>
        <tr><td style="background:#fafafa;border-bottom:1px solid #eee">Phone</td><td style="border-bottom:1px solid #eee">${escape(phone || '—')}</td></tr>
        <tr><td style="background:#fafafa;border-bottom:1px solid #eee">Needs</td><td style="border-bottom:1px solid #eee">${escape(needsStr || '—')}</td></tr>
        <tr><td style="background:#fafafa;border-bottom:1px solid #eee">Message</td><td style="border-bottom:1px solid #eee">${escape(message || '—')}</td></tr>
        <tr><td style="background:#fafafa">Timestamp</td><td>${new Date().toISOString()}</td></tr>
      </table>
    </td></tr>
    ${sheetUrl ? `<tr><td style="padding-top:16px"><a href="${sheetUrl}" style="display:inline-block;padding:10px 14px;background:#111;color:#fff;text-decoration:none;border-radius:6px">Open Sheet</a></td></tr>` : ''}
  </table>`;
  const res = await resend.emails.send({ from, to, subject, html });
  if (!res || res.error) throw new Error(`Resend error: ${res?.error || 'unknown'}`);
  return res;
}
